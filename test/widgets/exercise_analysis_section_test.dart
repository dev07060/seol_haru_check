...
void main() {
  group('ExerciseAnalysisSection Widget Tests', () {
    late AIAnalysis testAnalysis;

    setUp(() {
      testAnalysis = const AIAnalysis(
        exerciseInsights: '이번 주 운동 패턴이 매우 일관적이었습니다. 특히 유산소 운동과 근력 운동의 균형이 좋았습니다.',
        dietInsights: '식단 관리가 잘 되고 있습니다.',
        overallAssessment: '전반적으로 우수한 진전을 보이고 있습니다.',
        strengthAreas: ['운동 일관성', '운동 다양성', '근력 운동 집중'],
        improvementAreas: ['운동 강도 증가', '유연성 운동 추가', '회복 시간 관리'],
      );
    });

    Widget createTestWidget(AIAnalysis analysis, {WeeklyStats? stats}) {
      return MaterialApp(
        home: Scaffold(
          body: ExerciseAnalysisSection(
            analysis: analysis,
            stats: stats,
            categoryTrends: null,
            exerciseCategoryData: null,
    ...
    group('Basic Display', () {
      testWidgets('should display section header with icon and title', (tester) async {
        ...
        expect(find.text(AppStrings.exerciseAnalysis), findsOneWidget);
        expect(find.byIcon(Icons.fitness_center), findsOneWidget);
      });

      testWidgets('should display exercise insights when available', (tester) async {
        await tester.pumpWidget(createTestWidget(testAnalysis));
        await tester.pumpAndSettle();

        expect(find.text('운동 패턴 분석'), findsOneWidget);
        expect(find.text('이번 주 운동 패턴이 매우 일관적이었습니다. 특히 유산소 운동과 근력 운동의 균형이 좋았습니다.'), findsOneWidget);
      });

      testWidgets('should not display insights when empty', (tester) async {
        final emptyInsightsAnalysis = testAnalysis.copyWith(exerciseInsights: '');

        await tester.pumpWidget(createTestWidget(emptyInsightsAnalysis));
        await tester.pumpAndSettle();

        expect(find.text('운동 패턴 분석'), findsNothing);
      });
    });

    group('Strength Areas Display', () {
      testWidgets('should display exercise-related strength areas', (tester) async {
        await tester.pumpWidget(createTestWidget(testAnalysis));
        await tester.pumpAndSettle();

        expect(find.text('운동 강점'), findsOneWidget);
        expect(find.text('운동 일관성'), findsOneWidget);
        expect(find.text('운동 다양성'), findsOneWidget);
        expect(find.text('근력 운동 집중'), findsOneWidget);
      ...
      testWidgets('should filter out non-exercise related strength areas', (tester) async {
        final mixedAnalysis = testAnalysis.copyWith(strengthAreas: ['운동 일관성', '식단 균형', '수면 패턴', '근력 운동']);
        ...
        expect(find.text('식단 균형'), findsNothing); // Should be filtered out
        expect(find.text('수면 패턴'), findsNothing); // Should be filtered out
      });

      testWidgets('should not display strength section when no exercise-related areas', (tester) async {
        final nonExerciseAnalysis = testAnalysis.copyWith(strengthAreas: ['식단 균형', '수면 패턴', '스트레스 관리']);

        await tester.pumpWidget(createTestWidget(nonExerciseAnalysis));
        await tester.pumpAndSettle();

        expect(find.text('운동 강점'), findsNothing);
      });
    ...
    group('Improvement Areas Display', () {
      testWidgets('should display exercise-related improvement areas', (tester) async {
        await tester.pumpWidget(createTestWidget(testAnalysis));
        await tester.pumpAndSettle();

        expect(find.text('운동 개선점'), findsOneWidget);
        expect(find.text('운동 강도 증가'), findsOneWidget);
        expect(find.text('유연성 운동 추가'), findsOneWidget);
        expect(find.byIcon(Icons.trending_up), findsOneWidget);

        // Check for multiple arrow icons (one for each improvement area)
        final arrowIcons = find.byIcon(Icons.arrow_upward);
        expect(arrowIcons.evaluate().length, greaterThanOrEqualTo(2));
      });

      testWidgets('should filter out non-exercise related improvement areas', (tester) async {
        final mixedAnalysis = testAnalysis.copyWith(improvementAreas: ['운동 강도', '식단 개선', '수분 섭취', '근력 훈련']);

        await tester.pumpWidget(createTestWidget(mixedAnalysis));
        await tester.pumpAndSettle();

        expect(find.text('운동 강도'), findsOneWidget);
        expect(find.text('근력 훈련'), findsOneWidget);
        expect(find.text('식단 개선'), findsNothing); // Should be filtered out
        expect(find.text('수분 섭취'), findsNothing); // Should be filtered out
      });

      testWidgets('should not display improvement section when no exercise-related areas', (tester) async {
        final nonExerciseAnalysis = testAnalysis.copyWith(improvementAreas: ['식단 개선', '수분 섭취', '수면 질']);

        await tester.pumpWidget(createTestWidget(nonExerciseAnalysis));
        await tester.pumpAndSettle();

        expect(find.text('운동 개선점'), findsNothing);
      });
    ...
    group('Visual Styling', () {
      ...
      testWidgets('should display header with green accent color', (tester) async {
        await tester.pumpWidget(createTestWidget(testAnalysis));
        await tester.pumpAndSettle();

        // Check for container with green background
    ...
    group('Layout Tests', () {
      testWidgets('should arrange sections vertically', (tester) async {
        ...
        final analysisHeader = find.text(AppStrings.exerciseAnalysis);
        final strengthHeader = find.text('운동 강점');
        final improvementHeader = find.text('운동 개선점');
    ...
    group('Edge Cases', () {
      testWidgets('should handle very long insight text', (tester) async {
        final longInsightAnalysis = testAnalysis.copyWith(exerciseInsights: '이것은 매우 긴 운동 분석 텍스트입니다. ' * 20);
      ...
      testWidgets('should handle single strength and improvement area', (tester) async {
        final singleItemAnalysis = testAnalysis.copyWith(strengthAreas: ['운동 일관성'], improvementAreas: ['운동 강도']);
