...
export class VertexAIService {
    ...
    constructor(
        ...
        location: string = "asia-northeast3",
        ...
        try {
            ...
            logger.info("VertexAI service initialized successfully", {
                projectId,
                location,
                modelName,
            });
            ...
            logger.error("Failed to initialize VertexAI service", {
                error: errorMessage,
                projectId,
                location,
                modelName,
            ...
            let userFriendlyMessage = "VertexAI 서비스 초기화에 실패했습니다.";

            if (errorMessage.includes("permission") || errorMessage.includes("unauthorized")) {
                userFriendlyMessage = "VertexAI 서비스 접근 권한이 없습니다.";
            } else if (errorMessage.includes("quota") || errorMessage.includes("limit")) {
                userFriendlyMessage = "VertexAI 서비스 사용량이 초과되었습니다.";
            } else if (errorMessage.includes("network") || errorMessage.includes("connection")) {
                userFriendlyMessage = "VertexAI 서비스 연결에 실패했습니다.";
            } else if (errorMessage.includes("project")) {
                userFriendlyMessage = "잘못된 프로젝트 설정입니다.";
            }
    ...
    private async generateWithRetry(
        ...
        try {
            ...
            const request = {
                ...
                generationConfig: {
                    temperature: config.temperature ?? VertexAIService.DEFAULT_GENERATION_CONFIG.temperature,
                    maxOutputTokens: config.maxOutputTokens ?? VertexAIService.DEFAULT_GENERATION_CONFIG.maxOutputTokens,
                    topP: config.topP ?? VertexAIService.DEFAULT_GENERATION_CONFIG.topP,
                    topK: config.topK ?? VertexAIService.DEFAULT_GENERATION_CONFIG.topK,
                },
            };
            ...
            if (!candidate.content || !candidate.content.parts || candidate.content.parts.length === 0) {
                throw new Error("No content in VertexAI candidate");
            }
...
- 작은 목표부터 시작해보세요`,
    ...
    async testConnection(): Promise<boolean> {
        try {
            ...
            return testResponse.text.includes("연결") || testResponse.text.includes("성공");
        } catch (error) {
