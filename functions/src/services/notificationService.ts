...
export class NotificationService {
    ...
    private static async checkForConsolidation(
        ...
        const existingNotifications = pendingNotifications.docs.map(doc => doc.id);
    ...
    private static async sendConsolidatedNotification(
        ...
        batch.update(db.collection('notificationStatus').doc(notificationId), {
            status: 'sent',
            sentAt: new Date(),
            fcmMessageId: messageId,
        ...
        for (const existingId of existingNotificationIds) {
            batch.update(db.collection('notificationStatus').doc(existingId), {
                status: 'consolidated',
    ...
    private static async createNotificationStatus(
        ...
        const notificationRef = db.collection('notificationStatus').doc();
    ...
    private static async updateNotificationStatus(
        ...
        await db.collection('notificationStatus').doc(notificationId).update(updates);
    }
     ...
     */
    static async cleanupOldNotifications(olderThanDays: number = 30): Promise<number> {
